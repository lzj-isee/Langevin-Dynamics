...
def SG_UL_MCMC_it(datas,num_epoch,dim,factor_a,factor_b,factor_gamma,u,gamma,lens):
    x_list=[]
    x=np.ones(dim)*0
    v=np.ones(dim)*0
    for epoch in tqdm(range(num_epoch)):
        for i ,data in enumerate(datas):
            ...
            #----
            eta=factor_a*(factor_b+epoch*lens+i+1)**(-factor_gamma)
...
def SG_UL_MCMC_sample(random_seed,train_setting,save_folder):
    ...
    #SGLD
    x_list=SG_UL_MCMC_it(datas,num_epoch,dim,factor_a,factor_b,factor_gamma,u,gamma,len(datas_np))
    #SAVE
