...
def SRM_HMC_it(datas,num_epoch,dim,batchSize,factor_a,factor_b,factor_gamma,u,gamma):
    Num=len(datas)
    t=0
    x_list=[]
    ...
    for epoch in tqdm(range(num_epoch)):
        for k in range(int(Num/batchSize)):
            ...
            g=grad_f(x,datas_choice).mean(0)+(1-1/t**(0.5))*(g-grad_f(x_last,datas_choice).mean(0))
...
def SRM_HMC_sample(random_seed,train_setting,save_folder):
    ...
    #SGLD
    x_list=SRM_HMC_it(datas,num_epoch,dim,batchSize,factor_a,factor_b,factor_gamma,u,gamma)
    #SAVE
