...
def _SVRG_LD_iter(lr_a,lr_gamma,num_epochs,train_set,train_loader,full_train_loader,\
    ...
    curr_iter_count=0.0
    param=np.zeros(dim)
    param_snap=np.zeros(dim)

    for epoch in range(num_epochs):
        param_snap=param
        alpha=grad_Calc((train_set.tensors[0]).numpy(),(train_set.tensors[1]).numpy(),param_snap).mean(0)
        for i, (features,labels) in enumerate(train_loader):
            curr_iter_count+=1
            features=features.numpy()
            labels=labels.numpy()
            grad=param+train_num*(grad_Calc(features,labels,param).mean(0)-\
                grad_Calc(features,labels,param_snap).mean(0)+alpha)
...
def SVRG_LD_trian(lr_a,lr_gamma,num_epochs,batchSize,print_interval,random_seed,save_folder):
    ...
    train_loss,train_corr,test_loss,test_corr=_SVRG_LD_iter(
