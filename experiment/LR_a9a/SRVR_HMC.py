...
def _SRVR_HMC_iter(lr_a,lr_gamma,friction,num_epochs,train_set,train_loader,full_train_loader,\
    ...
    for epoch in range(num_epochs):
        ...
        for i, (features,labels) in enumerate(train_loader):
            ...
            labels=labels.numpy()
            if  i!=0:
                g=g+(grad_Calc(features,labels,param).mean(0)-grad_Calc(features,labels,param_last).mean(0))*train_num
...
def SRVR_HMC_trian(lr_a,lr_gamma,friction,num_epochs,batchSize,print_interval,random_seed,save_folder):
    ...
    print('SRVR_HMC: lr_a:{}, lr_gamma:{}, friction:{}'.format(lr_a,lr_gamma,friction))
    train_loss,train_corr,test_loss,test_corr=_SRVR_HMC_iter(
        lr_a,lr_gamma,friction,num_epochs,train_set,train_loader,full_train_loader,test_set,test_loader,print_interval)
