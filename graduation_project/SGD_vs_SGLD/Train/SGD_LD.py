...
path.append('./')
from Optimizer.SGD_op import SGD_op
...
def _SGDLD_it(trainSet,testSet,writer,device,**kw):
    ...
    for epoch in tqdm(range(kw['num_epochs'])):
        for i in range(inner_loops):
            ...
            # Update
            optimizer1.step(eta=eta)
            # Eval and Print
            '''
            # SGD has no need to average
    ...
    optimizer2=SGLD_op(\
        ...
        device=device)
    model.burn_in=True
    print('Start to average')
    for epoch in tqdm(range(kw['num_epochs_2'])):
        for i in range(inner_loops):
            curr_iter_count+=1
            # Sample datas
            indices=np.random.choice(\
...
def SGDLD_train(**kw):
    ...
    # Main function
    _SGDLD_it(trainSet,testSet,writer,device,**kw)
