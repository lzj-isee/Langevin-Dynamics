...
def _RAISLD_it(trainSet,ref,save_path,**kw):
    x_list=[]
    ...
    model.save_figure(x_list,ref,save_path,'RAISLD')
    np.save(save_path,x_list)

def RAISLD_sample(**kw):
    # Creat the save folder and name for result
    save_name='RAISLD'+' '+\
        'lr[{:.2e},{:.2f},{:.2f}] alpha[{:.2f}] d[{:.1f}]'.format(\
            kw['lr_a'],kw['lr_b'],kw['lr_gamma'],kw['alpha'],kw['d'])
    if not os.path.exists(kw['save_folder']+'RAISLD'):
        os.makedirs(kw['save_folder']+'RAISLD')
    save_path=kw['save_folder']+'RAISLD/'+save_name
    ...
    ref=load_ref()
    # Main function
    _RAISLD_it(trainSet,ref,save_path,**kw)
